...
-- Keep explicit constants so spacing can be tuned without hunting literals.
local MATCH_RADIO_GAP_X  = 46 -- spacing between Match radios (Text vs Frame)
local ACTION_RADIO_GAP_X = 22 -- spacing between Action radios (about half of match)

-- Global "click-outside to unfocus" catcher for all edit boxes
local ActiveEditBox
local FocusCatcher = CreateFrame("Button", addonName.."FocusCatcher", UIParent)
FocusCatcher:SetAllPoints(UIParent)
FocusCatcher:SetFrameStrata("FULLSCREEN_DIALOG")
FocusCatcher:SetToplevel(true)
FocusCatcher:EnableMouse(true)
FocusCatcher:Hide()
FocusCatcher:SetScript("OnMouseDown", function()
    if ActiveEditBox and ActiveEditBox.ClearFocus then ActiveEditBox:ClearFocus() end
end)
...
local function AttachOutsideClickBlur(eb)
    ...
    -- Ensure FocusCatcher never remains stuck if the EB is hidden/detached while focused
...
local function CreateNamedCheckbox(name, parent, label)
    ...
    if l then
        l:SetText(label or "")
        l:ClearAllPoints()
        l:SetPoint("LEFT", cb, "RIGHT", 1, 0) -- tighter label-to-box spacing (was 6)
    end
    return cb, l
end
...
local function CreateSmallCheckbox(parent, labelText)
    ...
    label:SetPoint("LEFT", cb, "RIGHT", 1, 0) -- tighter label-to-box spacing (was 6)
    local function updateHitRect()
        local w = (label:GetStringWidth() or 0)
        cb:SetHitRectInsets(0, - (w + 1), 0, 0) -- include label in clickable area
    end
    updateHitRect()
...
-- ======================
-- General panel (Detect ElvUI removed; scroll-wheel option added)
-- ======================
do
    ...
    popupSelectCheck:SetPoint("TOPLEFT", showNumbersCheck, "BOTTOMLEFT", 0, -ROWGAP)
    ...
    postalCheck:SetPoint("TOPLEFT", popupSelectCheck, "BOTTOMLEFT", 0, -ROWGAP)

    -- NEW: scroll wheel selection option
...
do
    ...
    local actionHolder = CreateFrame("Frame", nil, createPanel); actionHolder:SetSize(180, 22)
    actionHolder:SetPoint("TOPLEFT", actionLabel, "BOTTOMLEFT", 0, -6)
    local actionDD = CreateNamedDropdown(addonName.."Create_ActionDD", actionHolder, 180)
    ...
    HideDropDownBlizzardTextures(actionDD); RestyleDropdown(actionDD, 180)
    ...
    prefillBtn:SetPoint("TOPLEFT", patEdit, "BOTTOMLEFT", 0, -TOPGAP)
    ...
    testBtn:SetPoint("TOPLEFT", prefillBtn, "BOTTOMLEFT", 0, -ROWGAP)
    ...
    fstackBtn:SetPoint("TOPLEFT", testBtn, "BOTTOMLEFT", 0, -ROWGAP)
    ...
    addBtn:SetScript("OnClick", function()
        ...
        if ok then
            -- Set just-added rule's name to the provided unique name
    ...
    prefillBtn:SetScript("OnClick", function()
        ...
        if frameName then
            ...
            actionState = 2; SetDropdownText(actionDD, "Button2 (2)"); RestyleDropdown(actionDD, 180)
            nameEdit:SetText("From "..frameName)
...
do
    ...
    local function CreateRuleCard(parent, index, rule, cardWidth)
        ...
        titleBtn:SetScript("OnClick", function()
            -- Build a full-width edit box that overlays the title row, aligned with the card's right edge
            local nameEB = CreateEditBox(card, (cardWidth or 400) - CARD_PAD*2)
            nameEB:ClearAllPoints()
            -- Left aligned with the existing title text
            nameEB:SetPoint("TOPLEFT", titleFS, "TOPLEFT", -2, 2)
            -- Right aligned to the card's inner edge
            nameEB:SetPoint("RIGHT", card, "RIGHT", -CARD_PAD, 0)
            -- Give it at least a comfortable row height
            ...
            -- Cancel with Escape (revert)
            nameEB:SetScript("OnEscapePressed", function(self)
                self:SetText(original)
                self:ClearFocus()
            end)
            -- Commit change on blur and ensure FocusCatcher is cleaned up even if we programmatically hide
            nameEB:HookScript("OnEditFocusLost", function(self)
                ...
                -- Ensure FocusCatcher clears if this EB is being removed while focused
                if ActiveEditBox == self then
                    ActiveEditBox = nil
                    FocusCatcher:Hide()
                end
        ...
        y = y - rowH1 - 12 -- more vertical space after the Match row
        ...
        y = y - patRowH - 12 -- a bit more spacing after pattern row
        ...
        if a == 1 then ra1:SetChecked(true) elseif a == 2 then ra2:SetChecked(true) else ra0:SetChecked(true) end
        if ra0._dkUpdateHitRect then ra0:_dkUpdateHitRect() end
        if ra1._dkUpdateHitRect then ra1:_dkUpdateHitRect() end
        if ra2._dkUpdateHitRect then ra2:_dkUpdateHitRect() end

        local rowH2 = ra0:GetHeight() or 18
        y = y - rowH2 - 12 -- slightly more vertical spacing after Action row
